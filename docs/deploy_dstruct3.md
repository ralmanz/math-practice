...
## Pre-flight — all six must pass

```bash
cd ~/Desktop/nuvo-studio/math-practice
node scripts/check-globals.js         # expect: "No global-scope collisions."
...
node scripts/test-frames.js           # expect: "All frame cases pass."
...
## Step 3 — Assign your account to Cambridge

The curriculum belongs to the **account**. There is no picker and no `?curriculum=` filter for a signed-in student.

```bash
node scripts/assign-curriculum.js ron CAMB s7 --dry   # inspect first
node scripts/assign-curriculum.js ron CAMB s7         # write it
```

Then confirm the record took it:

```bash
curl -s $W/student/ron | python3 -m json.tool | grep -E '"frame"|"band"'
```

The script refuses PROVISIONAL bands and unknown band ids, so a typo can't assign you to something that has no content.

---

## Step 4 — Look at it

Sign in as `ron` and open `home.html` with **no query string at all**:

```
https://nuvostudio.club/home.html
```

1. Badge reads **Cambridge — Stage 7**, not PAA. No URL param told it that — the account did.
2. Three unit cards: Algebraic expressions (2 levels), Linear equations (1 level).
3. `Sequences` does **not** appear — it has no seeds, so it isn't in the manifest.
4. Level 1 of the first unit is open; everything after it is locked.
5. Clicking a level lands on a canonical `?frame=CAMB&band=s7&…` URL.

Then the lesson itself:

1. Phase nav shows **two** steps (Intro → Practice), not three. No "Conceptos".
...
4. Problems are **English** — if any Spanish appears, stop and report it; the language filter has a hole.
...
- Do not hand-edit `frontend/content/lesson-manifest.json`. It is generated by `build-lessons.js`; editing it to reveal a level with no lesson record puts a 404 in front of a student.
- Do not add a curriculum picker. The account owns the curriculum — that is the whole point of `assign-curriculum.js`.
